"""
Based on
https://github.com/fredzzhang/pocket/blob/master/pocket/utils/meters.py

Modified for HICO recognition evaluation (to ignore uncertain labels)
...
class AveragePrecisionMeter:
    ...
    def __init__(self, num_gt: Optional[Iterable] = None,
            ...
            ignore: Optional[Tensor] = None) -> None:
        ...
        if is_none == (True, True):
            ...
            self._ignore = ignore.detach().cpu()
        else:
        ...
        self._ignore_temp = [torch.tensor([], dtype=bool)]
    ...
    @classmethod            
    def compute_ap(cls, output: Tensor, labels: Tensor, ignore: Tensor,
            num_gt: Optional[Tensor] = None,
            algorithm: str = 'AUC',
            chunksize: int = -1) -> Tuple[Tensor, Tensor, Tensor]:
        """
        ...
        Returns:
            ap(Tensor[K])
        """
        prec, rec = cls.compute_precision_and_recall(output, labels, ignore,
            num_gt=num_gt)
    ...
    @staticmethod
    def compute_precision_and_recall(output: Tensor, labels: Tensor, ignore: Tensor,
        ...
        # fix when both "tp[0, i]" and "fp[0, i]" are = 0, to avoid nan
        for i in range(tp.shape[1]):
            while tp[0, i] == 0 and fp[0, i] == 0:
                fp[0, i] = torch.finfo(float).eps
    ...
    def append(self, output: Tensor, labels: Tensor, ignore: Tensor) -> None:
        """
        ...
        if isinstance(output, torch.Tensor) and isinstance(labels, torch.Tensor):
            ...
            self._ignore_temp.append(ignore.detach().cpu())
        else:
    ...
    def reset(self, keep_old: bool = False) -> None:
        ...
        if not keep_old:
            self._output = torch.tensor([], dtype=self._dtype)
            self._labels = torch.tensor([], dtype=self._dtype)
            self._ignore = torch.tensor([], dtype=bool)
        self._output_temp = [torch.tensor([], dtype=self._dtype)]
        self._labels_temp = [torch.tensor([], dtype=self._dtype)]
        self._ignore_temp = [torch.tensor([], dtype=bool)]

    def eval(self) -> Tuple[Tensor, Tensor, Tensor]:
        """
        ...
        self._ignore = torch.cat([
            self._ignore,
            torch.cat(self._ignore_temp, 0)
        ])
        ...
        if len(self._output) and len(self._labels) and len(self._ignore):
            return self.compute_ap(self._output, self._labels, self._ignore, num_gt=self.num_gt,
...
class DetectionAPMeter:
    ...
    def __init__(self, num_cls: int, num_gt: Optional[Tensor] = None,
        ...
        if is_none == (True, True):
            ...
            self._output = _to_list_of_tensor(output, dtype=self._dtype, device='cpu')
            self._labels = _to_list_of_tensor(labels, dtype=self._dtype, device='cpu')
        else:
            raise AssertionError("Output and labels should both be given or None")
